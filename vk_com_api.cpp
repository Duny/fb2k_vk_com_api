...
{
    const GUID vk_api_provider::class_guid = externals::api_provider_class_guid;
    ...
    class api_url_builder
    {
        pfc::string8 m_url;
    public:
        api_url_builder (const char * p_method_name, const t_url_parameters & p_params)
        {
    ...
    class api_provider_impl : public vk_api_provider
    {
        ...
        response_json_ptr make_request (const char * p_api_name, const t_url_parameters & p_params, abort_callback & p_abort)
        {
        ...
        response_json_ptr invoke (const char * p_api_name, const t_url_parameters & p_params, abort_callback & p_abort) override
        {
            auto reset_timers = [&] () { m_first_call_time = m_last_call_time = GetTickCount (); };
            ...
            m_invoker_avaliable.wait_for (-1);
            {
                ...
                m_call_count = (m_call_count + 1) % vk_com_api::max_mp3_file_size;

                // No more than max_api_calls_per_second per second (1000 ms)
                if (m_call_count == 0) {
